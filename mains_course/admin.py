...
@admin.register(Course_Test_Series)
class Course_Test_SeriesAdmin(admin.ModelAdmin):
    list_display = ('title',)
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        form = super(Course_Test_SeriesAdmin, self).get_form(request, obj, **kwargs)
        return form

    def formfield_for_manytomany(self, db_field, request, **kwargs):
        if db_field.name == "moderator":
            kwargs["queryset"] = User.objects.filter(groups__name='instructor')
        return super(Course_Test_SeriesAdmin, self).formfield_for_manytomany(db_field, request, **kwargs)
